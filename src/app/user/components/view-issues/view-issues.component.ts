...
export class ViewIssuesComponent {
  ...
  dataSource = {
    getRows: (params: any) => {
      ...
      this.gitHubService.getIssuesUser(sortModel, filterModel, startRow, endRow, this.userId, '')
        .subscribe(data => {
  ...
  updateIssues(globalSearch: string): void {
    this.dataSource = {
      getRows: (params: any) => {
        ...
        this.gitHubService.getIssuesUser(sortModel, filterModel, startRow, endRow, this.userId, globalSearch)
          .subscribe({
